(function(){
  ...
  function indexSignals(){
    ...
    read(SIGNAL_KEY,[]).filter(Boolean).forEach(signal=>{
      ...
      if(!['search','share','store','view'].includes(signalKind))return;
      const subject=signalKind==='search'||signalKind==='store'?clean(signal.query||(signal.terms||[]).join(' ')||signal.title||signal.program):clean(signal.program||signal.title||signal.channel||signal.query);
      if(!subject)return;
      const kind=signalKind==='store'?'interest':signalKind,jobKey=`${kind}:${slug(signal.topicKey||subject)}`;
      const previous=byKey.get(jobKey)||{},at=Number(signal.lastAt||Date.parse(signal.collectedAt||signal.createdAt||'')||Date.now());
      byKey.set(jobKey,{...previous,jobKey,kind,subject,query:clean(signal.query||`${subject} ${signal.channel||''}`),sourceUrl:'',excludedSourceUrl:clean(signal.excludedSourceUrl||signal.url||signal.sourceUrl),excludedSourceUrls:[...new Set([...(previous.excludedSourceUrls||[]),signal.excludedSourceUrl,signal.url,signal.sourceUrl].map(clean).filter(Boolean))],originQuery:clean(signal.originQuery||signal.query),indexedText:'',collectedAt:new Date(at).toISOString(),signalCount:(Number(previous.signalCount)||0)+Math.max(1,Number(signal.hits)||1),status:previous.status==='published'?'published':'indexed'});
    });
